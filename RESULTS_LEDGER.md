...
### NB10 — results consolidation (2026-08-13)

| field | value |
|---|---|
| notebook | AG_PRAXIS_NB10_results_consolidation.ipynb |
| run date | 2026-08-13 |
| git sha | 5efdf16, with the notebook, its outputs and `tools/build_nb10.py` untracked at run time |
...
| environment | cpu, local. No accelerator, no Colab session, no Drive mount, no network call |
| reads | 27 artefacts across `results/NB05`, `results/NB07`, `results/NB09a`, `data/processed/NB03_verdict.json`, `data/processed/NB03b`, `data/processed/NB06`, `data/processed/NB08`, `data/processed/NB09a`, `data/processed/NB09b`, `data/processed/pairwise_auc.csv` and `config/shap_capec_map.yaml`. Every path is checked before anything is read and a missing input halts the run |
| writes | `results/chapter4`: 19 tables as CSV and Markdown, 3 figures as PNG at 300 dpi and as PDF, `captions.md` and `manifest.json` |
| agreement check | 43 of 43 emitted values match `PROJECT_RECORD.md` Section 5 and this ledger. The notebook halts on any mismatch and names the value |
| known divergence | `data/processed/NB09b/threat_mapping.json` records `pass_mark` 13, the criterion in force when it ran. `PROJECT_RECORD.md` states 15 of 18 under `PREREGISTRATION.md` Amendment 20, which followed that run. The notebook reports 15 and records the divergence in `manifest.json` rather than resolving it silently. The measured 18 of 18 clears both |
| figure numbering | none. Filenames are descriptive and numbering is added when the chapter is written |
| executed copy | `runs/AG_PRAXIS_NB10_results_consolidation_executed.ipynb` |

The notebook is generated by `tools/build_nb10.py`, which uses the standard library only.
The generator is the source and the notebook is its output, so an edit belongs in the
generator and the notebook is rebuilt from it.

### NB06d probes — DoS/DDoS confusion, read off saved predictions (2026-08-15)

Two scripts, run before the notebook and on the same saved arrays. They recompute every
figure from `y_true.npy` and `y_pred.npy` rather than reading it out of `metrics.json`,
and check what they compute against `metrics.json` afterwards.

| field | value |
|---|---|
| scripts | `scripts/dosicmp_confusion_probe.py`, `scripts/dosddos_pairs_probe.py` |
| run date | 2026-08-15 |
| status | reported diagnostic. Nothing trained, no metric introduced that is not recomputed from arrays already saved |
| environment | cpu, local. No accelerator, no Colab session, no Drive mount, no network call |
| reads | `data/processed/NB06/sequence_cnn_lstm_19class`, `results/NB05/ours_cnn_19class`, `results/NB05/published_cnn_19class`, and each run's own `metrics.json` for its label ordering |
| label ordering | all three runs carry the same ordering, so no remapping was needed |
| agreement check | every recomputed precision, recall, F1 and support agrees with the `metrics.json` it was checked against, across all 19 classes of all three runs. `all_disagreements` is empty |
| DoS-ICMP, sequence run | tp 936, fp 1,018, fn 3,000 on 3,936 windows; precision 0.479017, recall 0.237805, F1 0.317827 |
| DDoS-ICMP, sequence run | tp 6,809, fp 3,003, fn 1,017 on 7,826 windows; precision 0.693946, recall 0.870049, F1 0.772083 |
| where the DoS-ICMP errors go, sequence run | DDoS-ICMP 2,996 (76.118%), DDoS-UDP 3, Benign 1. 2,996 of 3,000 errors land on one class |
| where they go, record run on the same split | DDoS-ICMP 455 of 98,432 records (0.462%) |
| error concentration, all 19 classes, sequence run | 16 classes make any error; 11 of those send 90% or more of their errors to a single other class, and all 8 pair members are among the 11. Pair members median 0.9980, other classes median 0.7718 |
| artefacts | `data/processed/NB06d/dosicmp_confusion.json`, `data/processed/NB06d/dosddos_pairs.json` |

### NB06d — DoS and DDoS pair separability under window aggregation (2026-08-15)

| field | value |
|---|---|
| notebook | AG_PRAXIS_NB06d_dosddos_pair_diagnosis.ipynb |
| run date | 2026-08-15 |
| git sha | 05f673f, working tree clean |
| seed | 42 |
| pass reported | full, `is_fast_pass` false |
| status | reported diagnostic result. Nothing trained, no hypothesis tested, no comparator moved |
| environment | tensorflow 2.20.0, keras 3.13.2, numpy 2.0.2, pandas 2.2.2, backend tensorflow, accelerator cpu |
| reads | the saved test windows and the window model's saved predictions. Trains nothing |
| scored on | 49,159 test windows of 50 records over 44 features |
| measure | best-direction AUC per feature between the two members of a pair, taken three ways: pooled individual records, the window mean, and the window standard deviation |

Within-pair confusion, both models on the two-tier split, over the windows or records
whose true class is one of the pair.

| pair | record model | window model | factor | mean AUC lost | features above 0.90 on the window mean |
|---|---|---|---|---|---|
| DoS-ICMP / DDoS-ICMP | 0.001924 | 0.341183 | 177.3 | -0.067768 | 1 of 44 |
| DoS-TCP / DDoS-TCP | 0.000650 | 0.268424 | 413.1 | -0.042848 | 1 of 44 |
| DoS-SYN / DDoS-SYN | 0.000790 | 0.136305 | 172.6 | -0.008505 | 3 of 44 |
| DoS-UDP / DDoS-UDP | 0.000837 | 0.011398 | 13.6 | -0.053102 | 1 of 44 |

| field | value |
|---|---|
| swapped for the partner, window model | ICMP 4,013 of 11,762; TCP 2,841 of 10,584; SYN 1,477 of 10,836; UDP 134 of 11,756 |
| escaping the pair entirely, window model | ICMP 0.034%, TCP 0.094%, SYN 0.351%, UDP 0.000% |
| mean AUC lost | negative for all four pairs, so averaging the window raises mean separability rather than lowering it |
| features losing 0.10 or more to averaging | ICMP 0, TCP 0, SYN 1, UDP 0 |
| best feature on the window mean | IAT for ICMP 0.9601, TCP 0.9650 and UDP 0.9649; Variance for SYN 0.9840 with IAT second at 0.9711 |
| IAT per record | 0.9985 in all four pairs |
| IAT on the window spread | ICMP 0.5496, TCP 0.8035, SYN 0.6799, UDP 0.5650, below its figure on the mean in every pair |
| features flat on the mean that separate on the spread | ICMP 0 of 18, TCP 0 of 25, SYN 0 of 22, UDP 1 of 27 |
| ordering by separability lost | DoS-SYN > DoS-TCP > DoS-UDP > DoS-ICMP |
| ordering by confusion rate | DoS-ICMP > DoS-TCP > DoS-SYN > DoS-UDP |
