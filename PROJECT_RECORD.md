...
**Explainable Sequence-Based Intrusion Detection for Threat Modeling in
Internet of Medical Things Networks**
...
| Pass mark in the threat-mapping artefact, resolved | Recorded here on 2026-08-17 because this file never carried it as an open item, although it was one. `data/processed/NB09b/threat_mapping.json` held `pass_mark` 13, the criterion in force when NB09b ran on 2026-08-13, while Section 3 of this file states 15 of 18 under `PREREGISTRATION.md` Amendment 20, which followed that run. The divergence was tracked in the NB10 entry of `RESULTS_LEDGER.md` and reported in `results/chapter4/manifest.json` under `known_divergences` rather than reconciled, on the ground that reconciling it silently would hide that the criterion had changed. That reading stood for as long as the divergence did and is not superseded retrospectively. NB09b was re-run on 2026-08-17 and the artefact regenerated: `pass_mark` reads 15, and the artefact, both notebooks and this file now agree. Four fields changed in the artefact and no measured figure moved. NB10 was re-run in turn, so `known_divergences` is empty and the agreement check stands at 43 of 43. `tools/check_consistency.py` now reads the pass mark from the artefact and from both notebooks and fails if the three disagree, so this cannot reopen unnoticed. `RESULTS_LEDGER.md` carries both runs |
...
| Title and thesis statement, resolved | Settled on 2026-08-16. The title is *Sequence-Based Intrusion Detection and Threat Modeling for Internet of Medical Things Networks*. "Proactive" is dropped: it promised prediction ahead of impact, which the lookahead exclusion rules out. "Framework" is dropped: it promised a built artifact where the deliverable is a per-class threat model table. The thesis statement in Section 1 is restated to match, and the RO/RQ/H table in Section 3 is restated in the plain register of the NeCamp exemplar. No measured quantity moves: H1's class set and 0.50 threshold, H2's saturation comparator and epsilon, and H3's pass mark of 15 of 18 are unchanged and remain fixed in `PREREGISTRATION.md`. Revised on 2026-08-17 to *Explainable Sequence-Based Intrusion Detection for Threat Modeling in Internet of Medical Things Networks*. "Explainable" is added because RO3 is the objective that distinguishes this work, and the 2026-08-16 wording left it implicit: the deliverable is a per-class threat model table built from detector explanations, so the explanation step is what carries the contribution and the title should name it. "for Threat Modeling in" replaces "and Threat Modeling for" so the phrasing states that the detection serves the threat model rather than sitting beside it as a second, separate contribution. Again no measured quantity moves: the H1, H2 and H3 figures fixed in `PREREGISTRATION.md` are untouched by this revision |
