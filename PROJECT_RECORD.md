...
| NB09a executed, NB09b not executed | H3 is not yet evaluated. NB09a completed across two sessions: the five sequence models and their explainer passes on an A100 on 2026-08-12, and the forest's TreeExplainer pass on a CPU runtime on 2026-08-13, which also wrote `attributions.json`. All seven attribution files are present and the artefacts are in `data/processed/NB09a/` and `results/NB09a/`. `RESULTS_LEDGER.md` carries the measured figures under the completion entry of 2026-08-13, which supersedes the partial entry of 2026-08-12. NB09b has not been run, so the mapping, the agreement count against the reference standard, Kendall's tau and the MAUDE search are all outstanding. The rules they run under are fixed in `PREREGISTRATION.md` Amendments 18 and 19: the sequence model at 40 features is what H3 is scored on, the forest at 40 features is an exactness check on its approximate attributions, the aggregation is mean absolute, k is 10, the SHAP background is 200 windows held fixed across seeds, and nsamples is 50. The mapping rule and the reference standard are `config/stride_ground_truth.yaml`, `config/shap_capec_map.yaml`, `config/capec_stride.yaml` and `config/maude_keywords.yaml`, all committed before either notebook trains anything. 09a trains five sequence models and the forest and writes attributions per seed as each pass completes; 09b maps, counts assignment and agreement separately, computes Kendall's tau and queries openFDA, and runs on a CPU |
